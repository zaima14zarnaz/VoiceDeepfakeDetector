...
def train(model, train_loader, val_loader, optimizer, criterion, epochs, model_save_path, auc_fig_path, device):
    ...
    for epoch in range(epochs+1):
        ...
        for it, (xb, yb) in enumerate(train_loader):
            xb = xb.to(device, non_blocking=True)   # xb: [B, 1, T]
            yb = yb.to(device, non_blocking=True)
            xb = torch.nan_to_num(xb, nan=0.0, posinf=1e3, neginf=-1e3)
            if epoch == 0 :
                assert xb.dim() == 3 and xb.size(1) == 1, xb.shape
        ...
        with torch.no_grad():
            for it, (xb, yb) in enumerate(val_loader):
                xb = xb.to(device, non_blocking=True)   # xb: [B, 1, T]
                yb = yb.to(device, non_blocking=True)
                if epoch == 0 and it == 0:
                    assert xb.dim() == 3 and xb.size(1) == 1, xb.shape
