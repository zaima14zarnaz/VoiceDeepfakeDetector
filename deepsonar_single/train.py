...
def train(model, train_loader, val_loader, optimizer, criterion, epochs, model_save_path, device):
    ...
    for epoch in range(epochs+1):
        ...
        # Load a batch of input tensors (x1) and labels (yb) from the training data loader
        for x1, yb in train_loader:
            x1, yb = x1.to(device), yb.to(device) # Move all tensors to the GPU or CPU based on the selected device
            ...
            preds = model(x1) # Perform a forward pass through the model to get predictions
        ...
        with torch.no_grad(): # Disable gradient calculation to reduce memory usage and speed up inference
            ...
            for x1, yb in val_loader:
                x1, yb = x1.to(device), yb.to(device) # Move the batch to the appropriate device (GPU or CPU)
                x1 = x1.view(x1.size(0), -1) # Flatten x1 into a 2D tensor for model input
                preds = model(x1) # Perform a forward pass through the model to obtain predictions
